...
\section{Uniform forgetting properties}
...
\end{align*}
The elements $(X^{n,N}_{q+1\link q+1},X^{n,N}_{q\link q+1})_{0\le q \le \ordermax{n}{N}}$ are described in the case of the round-robin scheduling in Section~\ref{sec:round:robin}.
...
\end{figure}
The analysis of the asymptotic properties of the maximum likelihood estimator to Bradley-Terry models in random environment is established using the geometric decaying mixing rate of the conditional law of the players $(V^{n,N}_{k})_{0\le k \le \ordermax{n}{N}+1}$ given the observations $X^{n,N}_{q:\ordermax{n}{N}-1}$ for any $0\le q< \ordermax{n}{N}$.
% which is based on the graphical model displayed in Figure~\ref{fig:robin:graphicalmodel}.
%\begin{align*}
...
%\end{hypH}
Lemma~\ref{lem:minorization} is proved in Section~\ref{sec:proofs}.
\begin{lemma}
...
\section{Uniform convergence of the loglikelihood}
...
\end{figure}
As described in Section~\ref{sec:round:robin}, when the players meet according to the round-robin scheduling, $X_{2:\ordermax{n}{N}-1}^{n,N}$ is a stationary sequence while the observations $X_{0}^{n,N}$, $X_{1}^{n,N}$, $X_{\ordermax{n}{N}}^{n,N}$ do not behave similarly as contestants in $V^{n,N}_{1}$ and in $V^{n,N}_{\ordermax{n}{N}}$ play a specific role. Therefore, the loglikelihood of the observations may be decomposed as follows:
\[
...
%boils down to 
%\[
% \varepsilon^{-(c_1+c_2)n^2}\frac{\left(1-\varepsilon^{c_2n^2}\right)^{\ordermax{n}{N}}-\left(1-\varepsilon^{c_2n^2}\right)^{}}{\ordermax{n}{N}}\limit{N} 0\quad \Pstar\mbox{-a.s.}\eqsp.
%\]

By \eqref{eq:bound:forget}, the sequence $(\log p_{\pi}^{n,N}(X_{q}^{n,N}|X_{q+1:\ordermax{n}{N}-1}^{n,N}))_{N\ge 1}$ converges uniformly in $\pi\in\Pi$, $\Pstar$-a.s. Denote by $\ell^{\infty}_{q,n}(\pi)$ this limit: for all $n\ge 1$
\[
...
The risk of the estimator $\MLE^{n,N}$ is measured by the excess loss defined as follows:
\[
R^{\bayes}(\MLE^{n,N})=\kullback^{\bayes}(\bayes)-\kullback^{\bayes}(\MLE^{n,N})\enspace.
\]
This excess loss may be bounded above by construction:
By construction
\begin{align*}
R^{\bayes}(\MLE^{n,N}) &=\kullback^{\bayes}(\bayes)-\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\bayes}^{n,N}\left(X_{\cA_{n,N}}\right)\right]\\
&\hspace{1cm}+\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\bayes}^{n,N}\left(X_{\cA_{n,N}}\right)\right]-\frac{1}{\ordermax{n}{N}}\log p_{\bayes}^{n,N}\left(X_{\cA_{n,N}}\right)\\
&\hspace{1cm}+\frac{1}{\ordermax{n}{N}}\log p_{\bayes}^{n,N}\left(X_{\cA_{n,N}}\right)-\frac{1}{\ordermax{n}{N}}\log p_{\MLE^{n,N}}^{n,N}\left(X_{\cA_{n,N}}\right)\\
&\hspace{1cm}+\frac{1}{\ordermax{n}{N}}\log p_{\MLE^{n,N}}^{n,N}\left(X_{\cA_{n,N}}\right)-\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\MLE^{n,N}}^{n,N}\left(X_{\cA_{n,N}}\right)\right]\\
&\hspace{1cm}+\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\MLE^{n,N}}^{n,N}\left(X_{\cA_{n,N}}\right)\right]-\kullback^{\bayes}(\MLE^{n,N})\eqsp,\\
&\le 2\sup_{\pi\in\Pi\cup\{\pi^*\}}\left|\kullback^{\bayes}(\pi)-\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]\right|\\
&\hspace{1cm}+2\sup_{\pi\in\Pi\cup\{\pi^*\}}\left|\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]-\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right|\enspace.
\end{align*}
For all $\pi\in\Pi$,
\begin{multline*}
\left|\kullback^{\bayes}(\pi)-\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]\right| \le \frac{1}{\ordermax{n}{N}} \E_{\bayes}\left[\sum_{q=2}^{\ordermax{n}{N}-1}\left|\ell^{\infty}_{2,n}(\pi)- \log p_{\pi}^{n,N}\left(X_{q}^{n,N}\middle|X_{q+1:\ordermax{n}{N}-1}^{n,N}\right)\right|\right]\\
+ \frac{1}{\ordermax{n}{N}} \E_{\bayes}\left[\left|2\ell^{\infty}_{2,n}(\pi)- \log p_{\pi}^{n,N}\left(X_{0}^{n,N},X_{1}^{n,N},X_{\ordermax{n}{N}}^{n,N}\middle|X_{2:\ordermax{n}{N}-1}^{n,N}\right)\right|\right]\eqsp.
\end{multline*}
Then, by Lemma~\ref{lem:exp:forget} and \eqref{eq:linfty}, for a round-Robin algorithm, there exists $c(n)$ such that, \textcolor{red}{on peut detailler c(n) si necessaire}
\[
2\sup_{\pi\in\Pi\cup\{\pi^*\}}\left|\kullback^{\bayes}(\pi)-\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]\right|\le \frac{c(n)}{\ordermax{n}{N}}\enspace.
\]
This yields:
\[
R^{\bayes}(\MLE^{n,N}) \le  \frac{c(n)}{\ordermax{n}{N}} + 2\sup_{\pi\in\Pi\cup\{\pi^*\}}\left|\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]-\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right|\eqsp.
\]
The last term is controlled following \cite{dedecker:gouezel:2015}. Define
\[
K(X_{\cA_{n,N}}) = \sup_{\pi\in\Pi\cup\{\pi^*\}}\left|\E_{\bayes}\left[\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right]-\frac{1}{\ordermax{n}{N}}\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right)\right|\eqsp.
\]
...
\subsection{Cardinality of each cell}
...
\begin{proof}[Proof of Lemma~\ref{lem:exp:forget}]
Note that,
\begin{align*}
