...
\begin{lemma}\label{lem:ElementsAtDistanceq} Assume that $2\le n<N/4$ and let $N/2-1= \ordermax{n}{N}(n-1)+r_0$ where $r_0<n-1$. Define $V_{0}^{n,N}=\{v_1\}$. One has
...
V_{\ordermax{n}{N}+1}^{n,N}=\{v_{2x+1} : x\in[(\ordermax{n}{N}-1)(n-1)+1,\ordermax{n}{N}(n-1)+r_0]\}\\
\cup\{v_{2x} : x\in[2+\ordermax{n}{N}(n-1),1+r_0+\ordermax{n}{N}(n-1)]\}\enspace.
\end{multline}
\end{lemma}
...
\eqref{def:Vq01'} is a direct consequence of the round-robin procedure. 
\end{proof}
