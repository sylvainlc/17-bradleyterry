...
\node[V4] (v16) [right=of v14] {${\scriptscriptstyle N-6}$};
...
\node[legend_V2] [below right=of v6] {\small{\textsc{$V_2^{3,N}$}}};
\node[legend_V3] [below right=of v10] {\small{\textsc{$V_3^{3,N}$}}};
\node[legend_V4] [below right=of v14] {\small{\textsc{$V_{\ordermax{3}{N}-1}^{3,N}$}}};
\node[legend_V5] (leg) [below right=of v18] {\small{\textsc{$V_{\ordermax{3}{N}}^{3,N}$}}};
\node[legend_V6] [above right =of v15] {\small{\textsc{$V_{\ordermax{3}{N}+1}^{3,N}$}}};
\end{tikzpicture}
...
\node[V3] (v12) [right=of v10] {$12$};
\node[V3] (v14) [right=of v12] {$14$};
\node[V4] (v24) [right=of v14] {${\scriptscriptstyle N-8}$};
...
%\]
Consider the following notations, for $1\le q \le \ordermax{n}{N}-1$,
\begin{align*}
...
X_{\ordermax{n}{N}}^{n,N} & = X_{\ordermax{n}{N} - \ordermax{n}{N} +1,e}^{n,N}\cup X_{\ordermax{n}{N}+1-\ordermax{n}{N}+1}^{n,N}\cup X_{\ordermax{n}{N}-\ordermax{n}{N}+1,o}^{n,N}\eqsp.
\end{align*}
The loglikelihood of the observations may be decomposed as follows:
\[
\log p_{\pi}^{n,N}\left(X_{\cA_{n,N}}\right) = \log p_{\pi}^{n,N}\left(X_{\ordermax{n}{N}}^{n,N}\right) + \sum_{q=2}^{\ordermax{n}{N}-1}  \log p_{\pi}^{n,N}\left(X_{q}^{n,N}\middle|X_{q+1:\ordermax{n}{N}}^{n,N}\right) +  \log p_{\pi}^{n,N}\left(X_{0}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right)\eqsp.
\]
%\begin{align*}
...
For all $n,N$, %conditional  $(X_{2:\ordermax{n}{N}}^{n,N},X_{\mathsf{end}}^{n,N})$, $(V_{q}^{n,N})_{q=\ordermax{n}{N}}^2$ is a Markov chain with transition kernel given, 
...
\]
where $X^{n,N}_{q- q+1} = X^{n,N}_{q- q+1,e}\cup X^{n,N}_{q- q+1,o}$. In addition,  for all $1\le q \le \ordermax{n}{N}$ there exists a measure $\mu^{n,N}_{\pi,q}$ such that:
\[
\mathbb{P}_{\pi}\left(V^{n,N}_{q}\in A\middle|V^{n,N}_{q+1},X_{2:\ordermax{n}{N}}^{n,N}\right)\ge \left(\frac{\varepsilon}{1+\varepsilon}\right)^{\card\left(X^{n,N}_{q- q+1}\right)} \mu^{n,N}_{\pi,q}(A)\eqsp.
\] 
\end{lemma}
%By \eqref{eq:bound:likelihood}, Lemma~\ref{lem:minorization}
\begin{proof}
For all $n,N$, for all $1\le q \le \ordermax{n}{N}$ and all measurable set $A$:
\begin{align*}
\mathbb{P}\left(V^{n,N}_{q}\in A\middle|V^{n,N}_{q+1:\ordermax{n}{N}+1},X_{2:\ordermax{n}{N}}^{n,N}\right) &\\
 &\hspace{-3.2cm}= \mathbb{P}\left(V^{n,N}_{q}\in A\middle|V^{n,N}_{q+1},X^{n,N}_{2:q-1},X^{n,N}_{q- q+1}\right)\eqsp,\\
 &\hspace{-3.2cm}= \frac{\int \1_{A}(v^{n,N}_{q})\pi(\rmd v^{n,N}_{q})\prod_{X_{i,j}\in X^{n,N}_{q- q+1}}\condlik(X_{i,j},v_{\{i,j\}})p_{\pi}^{n,N}(X^{n,N}_{2:q-1}|v^{n,N}_{q})}{\int \pi(\rmd v^{n,N}_{q})\prod_{X_{i,j}\in X^{n,N}_{q- q+1}}\condlik(X_{i,j},v_{\{i,j\}})p_{\pi}^{n,N}(X^{n,N}_{2:q-1}|v^{n,N}_{q})}\eqsp.
\end{align*}
By \eqref{eq:bound:likelihood},
\[
\mathbb{P}\left(V^{n,N}_{q}\in A\middle|V^{n,N}_{q+1},X_{2:\ordermax{n}{N}}^{n,N}\right)
\ge \left(\frac{\varepsilon}{1+\varepsilon}\right)^{\card\left(X^{n,N}_{q- q+1}\right)}\frac{\int \1_{A}(\rmd v^{n,N}_{q})\pi(v^{n,N}_{q})p_{\pi}^{n,N}(X^{n,N}_{2:q-1}|v^{n,N}_{q})}{\int \pi(\rmd v^{n,N}_{q})p_{\pi}^{n,N}(X^{n,N}_{2:q-1}|v^{n,N}_{q})}\eqsp. 
\]
...
For all $2\le q \le \ordermax{n}{N}$, all distribution $\lambda$ on $(\varepsilon,1]^{\card(V_{\ordermax{n}{N}}^{n,N})}$ and all measurable set $A$ define:
\begin{equation}
...
\end{equation}
Then, for all $2\le q \le \ordermax{n}{N}$, the conditional distribution of $V_{q}^{n,N}$ given $X_{q:\ordermax{n}{N}}^{n,N}$ may be written, for all measurable set $A$:
\begin{align*}
...
\begin{align}
\left|\log p_{\pi}^{n,N}\left(X_{\ordermax{n}{N}}^{n,N}\right)\right|&\le \card\left(X_{\ordermax{n}{N}}^{n,N}\right)\log \left(1+\frac{1}{\varepsilon}\right)\eqsp,\label{eq:bound:beg}\\
\left|\log p_{\pi}^{n,N}\left(X_{0}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right)\right|&\le \card\left(X_{0}^{n,N}\right)\log \left(1+\frac{1}{\varepsilon}\right)\label{eq:bound:end}\eqsp.
\end{align}
In addition,  for all $\ell>1$,  $2\le q< \ordermax{n}{N}$ and all $\pi\in\Pi$,
\begin{multline}
\left|\log p_{\pi}^{n,N}\left(X_{q}^{n,N}\middle|X_{q+1:\ordermax{n}{N}}^{n,N}\right) - \log p_{\pi}^{n,N+\ell}\left(X_{q}^{(n,N+\ell)}\middle|X_{q+1:\ordermax{n}{N+\ell}}^{n,N+\ell}\right) \right|\\
\leq \left(1+\frac{1}{\varepsilon}\right)^{\card\left(X^{n,N}_{q}\right)}\rho_{\varepsilon}^{\ordermax{n}{N}-q-2} \eqsp,\label{eq:bound:forget}
\end{multline}
where
\begin{equation}
\label{eq:defrho}
\rho_{\varepsilon} = 1-\left(\frac{\varepsilon}{1+\varepsilon}\right)^{\card\left(X^{n,N}_{q- q+1}\right)}\eqsp.
\end{equation}
In addition,
\begin{equation}
...
\begin{align*}
p_{\pi}^{n,N}\left(X_{0}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right) &= \int p_{\pi}^{n,N}\left(X_{0}^{n,N},v_{0}^{n,N},v_{1}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right)\rmd v_{0}^{n,N}\rmd v_{1}^{n,N}\eqsp,\\
&=\int p_{\pi}^{n,N}\left(v_{0}^{n,N},v_{1}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right) p_{\pi}^{n,N}\left(X_{0}^{n,N}\middle|v_{0}^{n,N},v_{1}^{n,N}\right)\rmd v_{0}^{n,N}\rmd v_{1}^{n,N}\eqsp,\\
&=\int p_{\pi}^{n,N}\left(v_{0}^{n,N},v_{1}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right) \prod_{X_{i,j}\in X_{0}^{n,N}}\condlik(X_{i,j},v_{\{i,j\}})\rmd v_{0}^{n,N}\rmd v_{1}^{n,N}\eqsp.
\end{align*}
By \eqref{eq:bound:likelihood},
\[
 \left(\frac{\varepsilon}{1+\varepsilon}\right)^{\card\left(X_{0}^{n,N}\right)}\le p_{\pi}^{n,N}\left(X_{0}^{n,N}\middle|X_{2:\ordermax{n}{N}}^{n,N}\right) \le 1\eqsp,
\]
so that \eqref{eq:bound:end} holds. By \eqref{eq:filt} and \eqref{eq:lastdist}, for all $2\le q< \ordermax{n}{N}$,
\begin{equation}
...
\begin{align*}
\left|p_{\pi}^{n,N}\left(X_{q}^{n,N}\middle|X_{q+1:\ordermax{n}{N}}^{n,N}\right) -  p_{\pi}^{(n,N+\ell)}\left(X_{q}^{(n,N+\ell)}\middle|X_{q+1:\ordermax{n}{N+\ell}}^{(n,N+\ell)}\right) \right|&\\
&\hspace{-4.5cm}\leq \rho_{\epsilon}^{\ordermax{n}{N}-q-2} \sup_{v^{n,N}_{q+1}}\left|\int\pi(\rmd v^{n,N}_q)\prod_{X_{i,j}\in X_{q}^{n,N}}\condlik(X_{i,j},v_{\{i,j\}})\right|\eqsp,\\
&\hspace{-4.5cm}\le\rho_{\epsilon}^{\ordermax{n}{N}-q-2} \eqsp,
\end{align*}
