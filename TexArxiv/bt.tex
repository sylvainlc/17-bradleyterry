...
\section{Uniform convergence of the round-robin scheduling based loglikelihood}
\label{sec:likelihood}
The log-likelihood of the observations is given, for all $\pi\in\Pi$, by:
\[
\Lo{\cD_{n,N}}{\pi}:=-\log\cro{p^{\pi}(X_{\cA_{n,N}})},\quad \text{where}\quad p^{\pi}(X_{\cA_{n,N}})=\int  \pa{\prod_{(i,j)\in \cA_{n,N}}k(X_{i,j},v_{\{i,j\}})}\pi^{\otimes N}(dv)\enspace.
\]
...
%\]
As described in Section~\ref{sec:round:robin}, when the players meet according to the round-robin algorithm, $X_{2:\ordermax{n}{N}-1}^{n,N}$ is a stationary sequence while the observations $X_{0}^{n,N}$, $X_{1}^{n,N}$, $X_{\ordermax{n}{N}}^{n,N}$ do not behave similarly. Therefore, the loglikelihood of the observations may be decomposed as follows:
\[
...
&\hspace{-3.5cm}\leq \left(\prod_{k=q+1}^{\ordermax{n}{N}-2}\rho_k(\epsilon)\right) \sup_{v^{n,N}_{q+1}}\left|\int\pi(\rmd v^{n,N}_q)\prod_{X_{i,j}\in X_{q}^{n,N}}\condlik(X_{i,j},v_{\{i,j\}})\right|\\
&\hspace{-3.5cm}\le \prod_{k=q+1}^{\ordermax{n}{N}-2}\rho_k(\epsilon)\eqsp,
\end{align*}
